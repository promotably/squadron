...
if [ -z "$skip_integration_tests" ]; then
    ( ./jenkins-integration-tests-squadron.sh "$stack_name" 2>&1 || touch ../test_failure ) \
        | tee $integration_test_results
    set -x
fi

$awscmd s3 cp $integration_test_results "s3://$ARTIFACT_BUCKET/$CI_NAME/$PROJECT/$CI_COMMIT_ID/integration-results-squadron.txt"
