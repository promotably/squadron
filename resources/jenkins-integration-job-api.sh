...
if [ -z "$skip_integration_tests" ]; then
    # pull down api sources
    $awscmd s3 cp "s3://$ARTIFACT_BUCKET/$CI_NAME/api/$api_ref/source.zip" api-source.zip

    # api integration tests
    mkdir api
    cd api
    unzip ../api-source.zip
    ( ../jenkins-integration-tests-api.sh "$stack_name" 2>&1 || touch ../test_failure ) \
        | tee $integration_test_results
    cd ..
fi

$awscmd s3 cp $integration_test_results "s3://$ARTIFACT_BUCKET/$CI_NAME/$PROJECT/$CI_COMMIT_ID/integration-results-api.txt"
